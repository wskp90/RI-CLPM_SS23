...
```{r}
clpmg.fit <- lavaan::sem(clpm.def, data=semdat, missing="ML", estimator = "MLR",mimic="Mplus", meanstructure = T, int.ov.free = T, group="pol_int")
```

### Modell Fit
```{r}
broom::glance(clpmg.fit) %>% select(c(cfi,tli,agfi,rmsea,srmr)) %>% 
  gt %>%
  fmt_number(decimals = 3, columns = c(cfi,tli,agfi,rmsea,srmr)) %>% gt_theme_nytimes()
```
...
broom::tidy(riclpm.fit) %>% filter(., op =="~") %>% select(., c(term,group, op, estimate,std.error,p.value,std.all)) %>% 
  gt %>% 
  fmt_number(decimals = 2, columns = c(estimate,std.error,std.all)) %>% fmt_number(decimals = 3, columns = p.value)
```
