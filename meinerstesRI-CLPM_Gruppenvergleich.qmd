...
### Koeeffizienten
```{r}
broom::tidy(clpmg.fit1) %>% filter(., op =="~") %>% select(., c(group,term,op, estimate,std.error,p.value,std.all)) %>% 
  gt %>% 
...
riclpm.fit1 <- lavaan::lavaan(riclpm.def, data=semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T, group = "pol_int")
riclpm.fit2 <- lavaan::lavaan(riclpm.def, data=semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T, group = "pol_int", group.equal = "regressions")
```
...
broom::glance(riclpm.fit) %>% select(c(cfi,tli,agfi,rmsea,srmr)) %>% 
  gt %>%
  fmt_number(decimals = 3, columns = c(cfi,tli,agfi,rmsea,srmr)) %>% gt_theme_nytimes()
broom::glance(riclpm.fit1) %>% select(c(cfi,tli,agfi,rmsea,srmr)) %>% 
  gt %>%
  fmt_number(decimals = 3, columns = c(cfi,tli,agfi,rmsea,srmr)) %>% gt_theme_nytimes()
broom::glance(riclpm.fit2) %>% select(c(cfi,tli,agfi,rmsea,srmr)) %>% 
  gt %>%
  fmt_number(decimals = 3, columns = c(cfi,tli,agfi,rmsea,srmr)) %>% gt_theme_nytimes()
lavTestLRT(riclpm.fit1,riclpm.fit2) %>% as.data.frame() %>% rownames_to_column(var = "Model") %>% gt %>% fmt_number(decimals = 3, columns = "Pr(>Chisq)")
```
