...
```{r}
DF$semdat = DF$orgdat %>%
  select(ends_with(c("_160p","_430i")) & starts_with(c("kp10","kp12","kp14")))
head(DF$semdat)
DF$semdat = DF$semdat %>% 
  mutate(across(everything(), ~rec(., rec = "-99,-97,-95,-93,-92,-71=NA;else = copy")))
...
# tic()
DF$semdat %>% 
  summary(.,maxsum = 20) %>% knitr::kable(.,format= 'markdown')
