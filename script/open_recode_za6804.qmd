...
#### Alle Issues
```{r}
DF$kp2_ip.mml = DF$za6838.sel %>% select(starts_with("kp2_sp","kp2_mig","kp2_kl")) %>% TAM::tam.mml(., verbose=FALSE) #Raschmodell schätzen
...
cat('Cronbachs alpha(KR-20):')
DF$za6838.sel %>% select(starts_with("kp2_ip")) %>% psych::alpha(., na.rm = TRUE) %>% summary
