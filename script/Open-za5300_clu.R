...
clu_d2 = clu_5301 %>% mutate(across(c("q2","q63","q64"), scale)) %>% dist(.,method = "euclidean") %>% as.dist()
...
hc34 = hcut(clu_5301, k = 6, stand=T, hc_func = "hclust", hc_method = "ward.D2", hc_metric = "euclidean")
fviz_cluster(hc32)
fviz_cluster(hc34)
...
clu_5301 = cbind(clu_5301,sub_grp1,sub_grp2,sub_grp3,sub_grp4)
...
table(clu_comp1$sub_grp3 , clu_comp1$sub_grp2) %>% rcompanion::cramerV()
...
clu_5301 %>% group_by(sub_grp1) %>% 
  summarise(m2 = mean(q2), m63 = mean(q63), m64 = mean(q64)) %>% 
  ggplot(aes(x=sub_grp1, y=m2)) + geom_line(aes(color = "PolInt")) +
  geom_line(aes(y = m63, color = "SozÖk")) +
  scale_y_continuous(sec.axis = sec_axis(~.*1, name="SozÖk"))
