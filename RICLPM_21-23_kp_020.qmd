...
DF$semdat = DF$semdat %>% rename(x1 = paste0(DF$setup$wave1,DF$setup$x), x2 = paste0(DF$setup$wave2,DF$setup$x), x3 = paste0(DF$setup$wave3,DF$setup$x),
                                 x4 = paste0(DF$setup$wave4,DF$setup$x), x5 = paste0(DF$setup$wave5,DF$setup$x),
                                 y1 = paste0(DF$setup$wave1,DF$setup$y), y2 = paste0(DF$setup$wave2,DF$setup$y), y3 = paste0(DF$setup$wave3,DF$setup$y),
                                 y4 = paste0(DF$setup$wave4,DF$setup$y), y5 = paste0(DF$setup$wave5,DF$setup$y))
```
...
```{r Estimate Models}
tic()
DF$lv_fit$cl$m5u <- lavaan(DF$modef$cl$m5u,  data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T)
DF$lv_fit$ri$m5u <- lavaan(DF$modef$ri$m5u,  data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T)
toc()
```
...
DF$lv_fit %>% map_depth(., 2, ~broom::glance(.)) %>% reduce(bind_rows) %>% unnest_wider(m5u) %>% select(c(cfi,tli,agfi,rmsea,srmr,AIC,BIC)) %>% 
...
### CLPM
```{r}
DF$cl_graph = prepare_graph(DF$lv_fit$cl$m5u, layout = DF$cl_layout) %>% edit_graph({label_location = 0.8}, element = "edges") %>% edit_graph({ label = paste(est_sig_std)}, element="edges") 
plot(DF$cl_graph)
```
### RI-CLPM
```{r}
DF$ric_graph = prepare_graph(DF$lv_fit$ri$m5u, layout = DF$RI_layout) %>% edit_graph({label_location = 0.8}, element = "edges") %>%edit_graph({ label = paste(est_sig_std)}, element="edges")
plot(DF$ric_graph)
```

### Crosslagged-Coeffs RI-CLPM
```{r}
broom::tidy(DF$lv_fit$ri$m5u) %>% filter(.,op =="~" & grepl("Lx", term) & grepl("Ly", term)) %>% 
