---
title: "GLES Panel"
subtitle: "AfD-Sympathie explained"
author:
- "Reimar ZEH"
...
pacman::p_load(sjlabelled,foreign,tidyverse,sjmisc,Hmisc,gt,gtExtras,gtsummary,panelr,
...
DF$vnam_list <- list(
  ...
  ief = c("_050k","_050h"),
  eef = c("_050a","_050e"))
DF$vwav_list <- list(tr = c(14,15,16),
  pp = c(14,15,17),
  ex = c(14,15,17),
  ief = c(14,15,16,17,18,19),
  eef = c(14,15,16,17,18,19))
...
# Define cns
DF$gles.scales$cns16 = c("kp16_030a","kp16_030b","kp16_030c","kp16_030d")
# Flatten list for select
...
DF$semdat = DF$orgdat %>%
  select("lfdn", "kp1_2601", "kpa2_2601","kp1_2320", "kpx_2280","kp9_2320", "kpa2_2320", "kpx_2290s","kpx_2290s.wa2",
         contains("_1621"),
         all_of(DF$allscales),
         starts_with(c("kp1_","kp14_","kp15_","kp16_","kp17_","kp18_","kp19_")) &
           ends_with(c("_010","_190bb","_430i","_1500","_020", "_770","_780", "_790","_1130","_1290")) & 
...
## Recode Vars
```{r Recode Vars}
DF$semdat %>% select(contains("2290s")) %>% frq
DF$semdat %<>% mutate(kpx_2290s = if_else(is.na(kpx_2290s), kpx_2290s.wa2, kpx_2290s))
...
# DF$semdat = DF$semdat %>% mutate(across(where(is.factor), ~ as_numeric(.)))

# SozioDemographie
DF$semdat = DF$semdat %>% mutate(male = rec(kpx_2280, rec = "2=0;else=copy"))

DF$semdat %<>% mutate(kp1_2320 = if_else(!is.na(kp1_2320)  & !is.na(kp9_2320), kp9_2320, kp1_2320),
                      kp1_2320 = if_else(is.na(kp1_2320) & !is.na(kpa2_2320), kpa2_2320, kp1_2320))
DF$semdat %<>% mutate(school = rec(kp1_2320, rec = "1,2=0;,3,4=1;5,9=2;else=copy"))


DF$semdat %<>% mutate(kp1_2601 = if_else(is.na(kp1_2601), kpa2_2601, kp1_2601))
DF$semdat %<>% mutate(wost = rec(kp1_2601, rec = "-95=NA;1:11=0;12:16=1"))
# DF$semdat %<>%  mutate(across(c(all_of(DF$allscales)), ~rec(., rec = "-99,-95,-93=NA; else = copy")))
...
# DF$semdat %<>% mutate(across(ends_with(c("_010","_190bb","_430i","_1500","_020", "_770","_780", "_790","_1130","_1290")), ~rec(., rec = "-99,-98,-97,-95,-93,-71=NA; else = copy"))) # %>%   column_to_rownames(var = "lfdn")
...
# DF$semdat %<>% mutate(across(contains("_1621"), ~rec(.,rec = "-99,-95,-93=NA; else = copy")))

# DF$semdat %>% select(where(is.numeric)) %>% descr(.,show = c("type", "n", "range"))
```
...
DF$afd_pred = c("wost","school","gjahr","male","kp17_010","tr16", "kp16_160p","pp17", "ex17", "ief17", "eef17","cns16","kp17_020","kp16_1500","kp17_1130","kp17_1290")
...
DF$lm$afd_sym3 = lm(kp17_430i ~ school + wost + male + gjahr + 
                      kp16_1500 + kp17_010 +
                      kp17_020 + tr16 + pp17 + ex17 + cns16, DF$semdat) 
DF$lm$afd_sym4 = lm(kp17_430i ~ school + wost + male + gjahr + 
                      kp16_1500 + kp17_010 +
                      kp17_020 + tr16 + pp17 + ex17_extrem + cnskp16 +
                      kp17_1130 + kp17_1290, DF$semdat) 

DF$lm$mtrust = lm(kp16_160p ~ school + wost + male + gjahr + 
                      kp16_1500 + kp16_010 +
                      kp16_020 + kp17_popul + kp17_extrem + kp16_consp +
                      kp17_1130 + kp17_1290, DF$semdat) 


DF$lm$afd_wahl = lm(kp17_190bb ~ school + wost + male + gjahr + 
                      kp16_1500 + kp17_010 +
                      kp17_020 + kp16_trust + kp17_popul + kp17_extrem + kp17_popul + kp16_consp +
                      kp17_1130 + kp17_1290, DF$semdat) 
DF$glm$afd_wahl = glm(kp17_190bb ~ school + wost + male + gjahr + 
                      kp16_1500 + kp17_010 +
                      kp17_020 + kp16_trust + kp17_popul + kp17_extrem + kp17_popul + kp16_consp +
                      kp17_1130 + kp17_1290, DF$semdat, family = 'binomial') 

# glance(DF$glm$afd_wahl)
# tidy(DF$glm$afd_wahl)

Modell = c("AfD_Sym_m1","AfD_Sym_m2","AfD_Sym_m3","AfD_Sym_m4","Media Trust","AfD_Wahl_m1")
...
# }
# 
# get_tidy_output <- function(x) {
#   lm.beta::lm.beta(x) %>%
#     tidy(conf.int = TRUE, conf.level = 0.5) %>%
#     gt() %>%
...
# }
# 
# DF$lm %>% map(function(x) list(get_glance_output(x), get_tidy_output(x)))
