...
## Select Vars
```{r}
DF$semdat_1 = DF$orgdat %>% select(contains(c("_1600","_1610")) & starts_with(c("kp17","kp18","kp19")))
head(DF$semdat_1)
...
DF$semdat_1 %>% select(contains(c("_1600"))) %>% 
                         mutate(across(everything(), ~rec(., rec = "-99,-95,-93=NA; else = copy"))) %>% irr::icc()
DF$semdat_1 %>% select(contains(c("_1610"))) %>% 
                         mutate(across(everything(), ~rec(., rec = "-99,-97,-95,-93=NA; else = copy"))) %>% irr::icc()
DF$semdat_1 %>% select(contains(c("_1610"))) %>% 
                         mutate(across(everything(), ~rec(., rec = "-99,-95,-93=NA;-97=1; else = copy"))) %>% irr::icc()
```
...
:::  {.hidden} 
## CLPM {.unlisted .unnumbered} 
```{r}
#| echo: false
#| include: false
DF$.int_sm.fit <- lavaan::sem(DF$clpm, data=DF$semdat_1, missing="ML", estimator = "MLR", mimic="Mplus", meanstructure = "default")
broom::glance(DF$.int_sm.fit) %>% select(., c(cfi,tli,agfi,rmsea,srmr,AIC,BIC)) %>% mutate(across(where(is.double), round, 3)) %>% knitr::kable(.,format= 'markdown')
