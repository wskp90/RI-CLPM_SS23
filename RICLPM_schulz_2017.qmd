...
#| warning: false
tic()
DF$sem.fit <- lavaan::sem(DF$clpm_L, data=DF$semdat, missing="listwise", estimator = "MLR", mimic="Mplus", meanstructure = T, group = "kp5_010")
broom::glance(DF$sem.fit) %>% 
...
#| warning: false
tic()
DF$ric.fit <- lavaan::sem(DF$riclpm, data=DF$semdat, missing="listwise", estimator = "MLR", mimic="Mplus", meanstructure = T, group = "grp_age")
broom::glance(DF$ric.fit) %>% 
  select(., c(cfi,tli,agfi,rmsea,srmr,estimator,nobs,norig,missing_method)) %>% 
  gt() %>% fmt_number(decimals = 3) %>% gt_theme_nytimes()
broom::tidy(DF$ric.fit) %>% filter(., op =="~") %>% 
