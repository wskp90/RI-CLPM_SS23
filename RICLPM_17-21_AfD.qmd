...
DF$semdat %>% 
  select(contains(c("x"))) %>% na.omit() %>% irr::icc(., model = "t", type = "c", unit = "s")
DF$semdat %>% 
...
DF$lv_fit %>% map_depth(., 2, ~broom::glance(.)) %>% reduce(bind_rows) %>% reduce(bind_rows) %>% 
  select(c(cfi,tli,agfi,rmsea,srmr,AIC,BIC)) %>% 
  gt %>% fmt_number(decimals = 3, columns = c(cfi,tli,agfi,rmsea,srmr)) %>% 
  cols_add(Modell = c("CLPM fixed","RI-CLPM fixed","CLPM free","RI-CLPM free")) %>% cols_move_to_start(columns = Modell) %>% gt_theme_nytimes()
