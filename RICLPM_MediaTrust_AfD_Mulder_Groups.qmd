...
```{r Select and Recode}
DF$semdat = DF$orgdat %>%
  select(ends_with(c("_160p","_430i")) & starts_with(c("kp9","kp10","kp12","kp14","kp15","kp16")), "kp1_2601", "kpa2_2601")
...
DF$semdat %<>% mutate(wost = cut(kp1_2601, breaks = c(1,11,Inf), labels = c("Wessi", "Ossi")))

DF$semdat %<>% select(!contains("2601"))
descr(DF$semdat, show = c("type", "n", "range"))
...
DF$lv_mod$cl$m6u <- lavaan(DF$modef$cl$m6u,  data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T, group = "wost")
# DF$lv_mod$ri$m6c <- lavaan::sem(DF$modef$ri$m6c, data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T)
DF$lv_mod$ri$m6u <- lavaan(DF$modef$ri$m6u,  data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T, group = "wost")

# DF$lv_mod$cl$m5c <- lavaan(DF$modef$cl$m5c, data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T)
DF$lv_mod$cl$m5u <- lavaan(DF$modef$cl$m5u,  data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T, group = "wost")
# DF$lv_mod$ri$m5c <- lavaan(DF$modef$ri$m5c, data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T)
DF$lv_mod$ri$m5u <- lavaan(DF$modef$ri$m5u,  data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T, group = "wost")

# DF$lv_mod$cl$m4c <- lavaan(DF$modef$cl$m4c, data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T)
DF$lv_mod$cl$m4u <- lavaan(DF$modef$cl$m4u,  data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T, group = "wost")
# DF$lv_mod$ri$m4c <- lavaan(DF$modef$ri$m4c, data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T)
DF$lv_mod$ri$m4u <- lavaan(DF$modef$ri$m4u,  data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T, group = "wost")

# DF$lv_mod$cl$m3c <- lavaan(DF$modef$cl$m3c, data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T)
DF$lv_mod$cl$m3u <- lavaan(DF$modef$cl$m3u,  data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T, group = "wost")
# DF$lv_mod$ri$m3c <- lavaan(DF$modef$ri$m3c, data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T)
DF$lv_mod$ri$m3u <- lavaan(DF$modef$ri$m3u,  data=DF$semdat, missing="ML", estimator = "MLR", meanstructure = T, int.ov.free = T, group = "wost")
toc()
```

## Compare Model Fit
```{r Model fit}
CL_Modell = c("CLPM 6 free","CLPM 5 free","CLPM 4 free","CLPM 3 free")
RI_Modell = c("RI-CLPM 6 free","RI-CLPM 5 free","RI-CLPM 4 free","RI-CLPM 3 free")
...
```
```{r eval=FALSE}
tmpgrf = prepare_graph(DF$lv_mod$ri$m3u, layout = DF$RI_layout) %>%
    edit_graph({label_location = 0.8}, element = "edges") %>%
    edit_graph({ label = paste(est_sig_std)}, element="edges")
plot(tmpgrf)
```
...
cl_crosslagged = DF$lv_mod$cl %>%  map(., function(x) {
  ...
  select(., c(term,op,group,estimate,std.error,p.value,std.all)) %>% gt %>% 
