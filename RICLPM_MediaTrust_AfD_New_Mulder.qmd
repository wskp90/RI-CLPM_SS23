...
```{r Select and Recode}
DF$semdat = DF$orgdat %>%
  select(ends_with(c("_160p","_160p1","_430i","_1500")) & starts_with(c("kp9","kp10","kp12","kp14","kp15","kp16","kp21")))
...
## Plot X
```{r}
summary_data <- DF$semdat %>% select(ends_with("160p")) %>% 
  pivot_longer(cols = starts_with("kp"), names_to = "variable", values_to = "value") %>% 
  mutate(time = as_numeric(str_extract(variable, "\\d+"))) %>% 
  group_by(time) %>%
  summarise(mean_value = mean(value, na.rm = TRUE),
...
DF$lv_mod$gm$m6y1 = lavaan::growth(DF$modef$gm$m6y1, data=DF$semdat, missing="ML", estimator = "MLR")
DF$lv_mod$gm$m6y2 = lavaan::growth(DF$modef$gm$m6y2, data=DF$semdat, missing="ML", estimator = "MLR")
DF$lv_mod$gm$m6y3 = lavaan::growth(DF$modef$gm$m6y3, data=DF$semdat, missing="ML", estimator = "MLR")
DF$lv_mod$gm$m6y4 = lavaan::growth(DF$modef$gm$m6y4, data=DF$semdat, missing="ML", estimator = "MLR")
DF$lv_mod$gm$m6y5 = lavaan::growth(DF$modef$gm$m6y5, data=DF$semdat, missing="ML", estimator = "MLR")
```
## Compare Model Fit
```{r G-Model Fit}
