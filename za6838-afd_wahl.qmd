...
DF$semdat = DF$orgdat %>% select("lfdn", starts_with(c("kp1_","kp17_","kp18_","kp19_")) & ends_with(c("_190bb","_430i","_1500","_020")) & !contains("flag") & !ends_with("y"))
```
...
broom::tidy(afd_wahl) %>% gt %>% fmt_number(decimals = 2) %>%
  tab_style(style = cell_fill(color = "lightgreen"), locations = cells_body(columns = c(term,estimate), rows = p.value < 0.05)) %>%
  gt_theme_nytimes()
broom::glance(afd_wahl) %>% gt %>% fmt_number(decimals = 2)
std_coff = lm.beta::lm.beta(afd_wahl)
