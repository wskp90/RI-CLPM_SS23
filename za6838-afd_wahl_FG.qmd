...
test = DF$orgdat %>% select("kp1_2601", "kpa2_2601")
test %>% frq
test %<>% mutate(across(c("kp1_2601","kpa2_2601"), ~ as.numeric(as.character(.))),
                 across(c("kp1_2601","kpa2_2601"), ~rec(., rec = "-99,-97,-95,-93,-92,-71=NA; else = copy")),
                 kp1_2601 = if_else(is.na(kp1_2601), kpa2_2601, kp1_2601))
test %>% frq


school = DF$orgdat %>% select("kp1_2320", "kp9_2320", "kpa2_2320")
school %>% frq
school %<>% mutate(across(c("kp1_2320","kp9_2320","kpa2_2320"), ~ as.numeric(as.character(.))))
school %<>% mutate(kp1_2320 = ifelse(kp1_2320 > 0 & kp9_2320 > 0, kp9_2320, kp1_2320))
school %<>% mutate(kp1_2320 = ifelse(kp1_2320 < 0 & kpa2_2320 > 0, kpa2_2320, kp1_2320))
school %>% frq

trust = DF$orgdat %>% select("kp16_160a", "kp16_160b", "kp16_160j", "kp16_160p", "kp16_160q")
trust %>% frq
trust %<>% mutate(across(everything(), ~rec(., rec = "-99,-97,-95,-93,-92,-71=NA; else = copy")))
trust %<>%  mutate(kp16_trust = ifelse(complete.cases(select(., "kp16_160a", "kp16_160b", "kp16_160j", "kp16_160p", "kp16_160q")), 
                                       rowSums(select(., "kp16_160a", "kp16_160b", "kp16_160j", "kp16_160p", "kp16_160q"), na.rm = TRUE), NA))




DF$semdat = DF$orgdat %>% select("lfdn", starts_with(c("kp1_","kp17_","kp18_","kp19_")) & ends_with(c("_190bb","_430i","_1500","_020", "_770","_780", "_790")) & !contains("flag") & !ends_with("y"))
```

## Recode Vars
```{r}
DF$semdat %<>% mutate(across(everything(), ~rec(., rec = "-99,-97,-95,-93,-92,-71=NA; else = copy"))) %>%   column_to_rownames(var = "lfdn")
...
```{r}
afd_wahl = lm(kp18_190bb ~ kp18_020 + kp1_1500 + kp18_430i + 
                kp1_1500*kp18_430i + kp18_020*kp18_430i, DF$semdat)

afd_sym = lm(kp18_430i ~ kp18_020 + kp1_1500 + kp18_780, DF$semdat)
